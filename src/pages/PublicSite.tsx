...
import { GRAD, GRAD_START, GRAD_MID, GRAD_END } from '@/lib/brand';
    ...
    <div style={{ background: '#000', color: '#fff', minHeight: '100vh' }}>
      ...
      <nav style={{
        ...
        <div className="flex items-center justify-between px-4 lg:px-8" style={{ maxWidth: 1400, margin: '0 auto', height: 72 }}>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map(l => (
              <a key={l.key} href={l.href} style={{ color: '#999', fontSize: 13, textDecoration: 'none', transition: 'color 0.3s', fontWeight: 300, letterSpacing: '0.05em' }}
                onMouseEnter={e => (e.currentTarget.style.color = GRAD_MID)} onMouseLeave={e => (e.currentTarget.style.color = '#999')}>
                {t(l.key)}
              </a>
            ))}
          </div>
          <div className="hidden lg:flex items-center gap-4">
            ...
            <a href="#contact" style={{ padding: '8px 20px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, textDecoration: 'none', letterSpacing: '0.1em' }}>
              {t('nav_quote')}
            </a>
          </div>
      ...
      <section id="home" style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        ...
        <div className="flex flex-col items-center justify-center text-center px-4" style={{ position: 'relative', zIndex: 2, height: '100%' }}>
          ...
          <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 200, ...gradTextStyle, marginBottom: 24 }}>{t('hero_sub')}</h2>
          <p style={{ color: '#999', fontSize: 15, maxWidth: 550, marginBottom: 40, fontWeight: 300, lineHeight: 1.8 }}>{t('hero_desc')}</p>
          <a href="#work" style={{ padding: '14px 40px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, textDecoration: 'none', letterSpacing: '0.15em', transition: 'opacity 0.3s' }}
            onMouseEnter={e => (e.currentTarget.style.opacity = '0.85')} onMouseLeave={e => (e.currentTarget.style.opacity = '1')}>
            {t('hero_cta')}
          </a>
          <div className="flex gap-2 mt-10">
            {heroImages.map((_, i) => (
              <button key={i} onClick={() => setHeroIdx(i)} style={{ width: heroIdx === i ? 32 : 8, height: 3, borderRadius: 2, background: heroIdx === i ? GRAD : 'rgba(255,255,255,0.2)', border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="work">
        <div style={{ padding: '80px 0 20px', textAlign: 'center' }}>
          <span className="grad-text" style={{ fontSize: 11, letterSpacing: '0.2em' }}>SELECTED WORK</span>
        ...
        {projects.map(p => (
          <div key={p.id} className="work-card" onClick={() => p.video_url && setVideoModal(p.video_url)}
            ...
            <div style={{
              ...
              <span className="grad-text" style={{ fontSize: 11, letterSpacing: '0.15em' }}>{p.category}</span>
      ...
      <section id="services" style={{ background: '#0a0a0a', paddingTop: 100, paddingBottom: 80 }}>
        <div className="px-4 lg:px-8" style={{ maxWidth: 1400, margin: '0 auto' }}>
          <div className="text-center mb-16">
            <span className="grad-text" style={{ fontSize: 11, letterSpacing: '0.2em' }}>WHAT WE DO</span>
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 200, marginTop: 12 }}>{t('services_title')}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {services.map(svc => (
              <div key={svc.num} className="flex gap-6 p-6 grad-border" style={{ borderRadius: 8, transition: 'all 0.3s', border: '1px solid #1a1a1a', background: '#000' }}
                onMouseEnter={e => { e.currentTarget.style.background = `linear-gradient(#000, #000) padding-box, ${GRAD} border-box`; e.currentTarget.style.border = '1px solid transparent' }}
                onMouseLeave={e => { e.currentTarget.style.border = '1px solid #1a1a1a'; e.currentTarget.style.background = '#000' }}>
                <span style={{ fontSize: 64, fontWeight: 200, ...gradTextStyle, lineHeight: 1, opacity: 0.3 }}>{svc.num}</span>
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <svc.icon size={22} style={{ color: GRAD_MID }} />
      ...
      <section id="about" className="px-4 lg:px-8" style={{ maxWidth: 1400, margin: '0 auto', paddingTop: 100, paddingBottom: 80 }}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <span className="grad-text" style={{ fontSize: 11, letterSpacing: '0.2em' }}>WHO WE ARE</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-16">
          {values.map(v => (
            <div key={v} className="text-center p-6" style={{ border: '1px solid #1a1a1a', borderRadius: 6 }}>
              <span className="grad-text" style={{ fontSize: 15, fontWeight: 400 }}>{t(v)}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="clients" style={{ background: '#0a0a0a', paddingTop: 80, paddingBottom: 80 }}>
        <div className="px-4 lg:px-8" style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div className="text-center mb-12">
            <span className="grad-text" style={{ fontSize: 11, letterSpacing: '0.2em' }}>TRUSTED BY</span>
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 200, marginTop: 12 }}>{t('clients_title')}</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {clients.map(c => (
              <div key={c} className="text-center p-5" style={{ border: '1px solid #1a1a1a', borderRadius: 6, transition: 'all 0.3s', cursor: 'default', background: '#000' }}
                onMouseEnter={e => { e.currentTarget.style.background = `linear-gradient(#000, #000) padding-box, ${GRAD} border-box`; e.currentTarget.style.border = '1px solid transparent' }}
                onMouseLeave={e => { e.currentTarget.style.border = '1px solid #1a1a1a'; e.currentTarget.style.background = '#000' }}>
      ...
      <section id="contact" className="px-4 lg:px-8" style={{ maxWidth: 800, margin: '0 auto', paddingTop: 100, paddingBottom: 80 }}>
        <div className="text-center mb-12">
          <span className="grad-text" style={{ fontSize: 11, letterSpacing: '0.2em' }}>GET IN TOUCH</span>
        ...
        {formSent && <div className="text-center p-4 mb-6" style={{ background: `${GRAD_START}15`, border: `1px solid ${GRAD_START}`, borderRadius: 6, color: GRAD_MID }}>{t('form_success')}</div>}
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={f.key}>
              ...
              <input type={f.type} required={f.key === 'full_name' || f.key === 'phone'}
                ...
                onFocus={e => (e.currentTarget.style.borderColor = GRAD_MID)} onBlur={e => (e.currentTarget.style.borderColor = '#1a1a1a')} />
            </div>
          ))}
          <div>
          ...
          <div className="md:col-span-2">
            <button type="submit" style={{ width: '100%', padding: '14px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer', letterSpacing: '0.15em', transition: 'opacity 0.3s' }}
              onMouseEnter={e => (e.currentTarget.style.opacity = '0.85')} onMouseLeave={e => (e.currentTarget.style.opacity = '1')}>
              {t('form_submit')}
            </button>
          </div>
      ...
      <footer style={{ background: '#000', borderTop: '1px solid #1a1a1a', paddingTop: 40, paddingBottom: 30 }}>
        <div className="px-4 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6" style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <Link to="/contact" className="grad-text" style={{ fontSize: 12, textDecoration: 'none', letterSpacing: '0.1em' }}>CONTACT →</Link>
        </div>
      ...
      <a href={`https://wa.me/${settings.whatsapp}`} target="_blank" rel="noopener"
        style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 90, width: 56, height: 56, borderRadius: '50%', background: GRAD, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(232,99,122,0.4)', transition: 'transform 0.3s' }}
