...
import { Menu, X, Play, Camera, Video, Plane, Radio } from 'lucide-react';
import { GRAD, TEAL, CORAL, PINK, BLUE_DEEP, BLUE_MID, BG, BORDER, WARM_GLOW, WARM_GLOW_HOVER, gradText as gradTextStyle } from '@/lib/brand';
    ...
    <div style={{ background: '#000', color: '#fff', minHeight: '100vh' }}>
        ...
        <div className="flex items-center justify-between px-4 lg:px-8" style={{ maxWidth: 1400, margin: '0 auto', height: 72 }}>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map(l => (
              <a key={l.key} href={l.href} style={{ color: '#999', fontSize: 13, textDecoration: 'none', transition: 'color 0.3s', fontWeight: 300, letterSpacing: '0.05em' }}
                onMouseEnter={e => (e.currentTarget.style.color = TEAL)} onMouseLeave={e => (e.currentTarget.style.color = '#999')}>
                {t(l.key)}
              </a>
            ))}
          </div>
          <div className="hidden lg:flex items-center gap-4">
            <button onClick={() => setLang(lang === 'en' ? 'ar' : 'en')}
              style={{ padding: '6px 16px', border: `1px solid ${BORDER}`, borderRadius: 4, background: 'transparent', color: '#999', fontSize: 12, cursor: 'pointer', letterSpacing: '0.1em', boxShadow: `0 0 6px rgba(248,112,96,0.15)`, transition: 'all 0.3s' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = CORAL; e.currentTarget.style.color = '#fff' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = BORDER; e.currentTarget.style.color = '#999' }}>
              {lang === 'en' ? 'AR' : 'EN'}
            </button>
            <a href="#work" style={{
              padding: '8px 20px', borderRadius: 4, fontSize: 12, fontWeight: 500, textDecoration: 'none', letterSpacing: '0.1em',
            ...
            onMouseEnter={e => { const el = e.currentTarget; el.style.background = GRAD; el.style.color = '#fff'; el.style.boxShadow = WARM_GLOW_HOVER }}
            onMouseLeave={e => { const el = e.currentTarget; el.style.background = `linear-gradient(#000, #000) padding-box, ${GRAD} border-box`; el.style.color = TEAL; el.style.boxShadow = WARM_GLOW }}>
              {t('hero_cta')}
            </a>
            <a href="#contact" style={{
              padding: '8px 20px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, textDecoration: 'none', letterSpacing: '0.1em',
              boxShadow: WARM_GLOW,
              transition: 'all 0.3s',
            }}
            onMouseEnter={e => { e.currentTarget.style.boxShadow = WARM_GLOW_HOVER }}
            onMouseLeave={e => { e.currentTarget.style.boxShadow = WARM_GLOW }}>
              {t('nav_quote')}
            </a>
          </div>
          ...
          <div className="lg:hidden px-4 pb-4" style={{ background: 'rgba(0,0,0,0.98)' }}>
            ...
            <div className="flex gap-3 mt-3">
              <button onClick={() => { setLang(lang === 'en' ? 'ar' : 'en'); setMenuOpen(false); }}
                style={{ padding: '6px 16px', border: `1px solid ${BORDER}`, borderRadius: 4, background: 'transparent', color: '#999', fontSize: 12, cursor: 'pointer' }}>
                {lang === 'en' ? 'AR' : 'EN'}
              </button>
              <a href="#contact" onClick={() => setMenuOpen(false)} style={{ padding: '8px 20px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, textDecoration: 'none' }}>
                {t('nav_quote')}
              </a>
            </div>
          </div>
      ...
      <section id="home" style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        ...
        <div className="flex flex-col items-center justify-center text-center px-4" style={{ position: 'relative', zIndex: 2, height: '100%' }}>
          <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.4em', fontWeight: 400, marginBottom: 20 }}>{t('hero_tag')}</span>
          <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)', fontWeight: 200, lineHeight: 1.1, marginBottom: 8 }}>{t('hero_main')}</h1>
          <h2 style={{ fontSize: 'clamp(2rem, 5vw, 4rem)', fontWeight: 200, marginBottom: 24, ...gradTextStyle }}>{t('hero_sub')}</h2>
          <p style={{ color: '#999', fontSize: 15, maxWidth: 550, marginBottom: 40, fontWeight: 300, lineHeight: 1.8 }}>{t('hero_desc')}</p>
          <a href="#work" style={{
            padding: '14px 40px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, textDecoration: 'none', letterSpacing: '0.15em',
            boxShadow: WARM_GLOW, transition: 'all 0.3s',
          }}
          onMouseEnter={e => { e.currentTarget.style.boxShadow = WARM_GLOW_HOVER }}
          onMouseLeave={e => { e.currentTarget.style.boxShadow = WARM_GLOW }}>
            {t('hero_cta')}
          ...
          <div className="flex gap-2 mt-10">
            {heroImages.map((_, i) => (
              <button key={i} onClick={() => setHeroIdx(i)} style={{ width: heroIdx === i ? 32 : 8, height: 3, borderRadius: 2, background: heroIdx === i ? TEAL : 'rgba(255,255,255,0.2)', border: 'none', cursor: 'pointer', transition: 'all 0.3s' }} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="work">
        <div style={{ padding: '80px 0 20px', textAlign: 'center' }}>
          <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.2em' }}>SELECTED WORK</span>
        ...
        {projects.map(p => (
          <div key={p.id} className="work-card" onClick={() => p.video_url && setVideoModal(p.video_url)}
            ...
            <div style={{
              ...
              <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.15em' }}>{p.category}</span>
      ...
      <section id="services" style={{ background: '#0a0a0a', paddingTop: 100, paddingBottom: 80 }}>
        <div className="px-4 lg:px-8" style={{ maxWidth: 1400, margin: '0 auto' }}>
          <div className="text-center mb-16">
            <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.2em' }}>WHAT WE DO</span>
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 200, marginTop: 12 }}>{t('services_title')}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {services.map(svc => (
              <div key={svc.num} className="flex gap-6 p-6" style={{
                border: `1px solid ${BORDER}`, borderRadius: 8, transition: 'all 0.3s',
              }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = TEAL; e.currentTarget.style.boxShadow = WARM_GLOW }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = BORDER; e.currentTarget.style.boxShadow = 'none' }}>
                <span style={{ fontSize: 64, fontWeight: 200, color: `rgba(45,212,191,0.12)`, lineHeight: 1 }}>{svc.num}</span>
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <svc.icon size={22} style={{ color: TEAL }} />
      ...
      {/* STATS */}
      <section ref={statsRef} style={{ background: '#000', paddingTop: 80, paddingBottom: 80, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 px-4 lg:px-8" style={{ maxWidth: 1200, margin: '0 auto' }}>
          {stats.map((s, i) => (
            <div key={i} className="text-center">
              <div style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 200, ...gradTextStyle, opacity: statsVisible ? 1 : 0, transform: statsVisible ? 'scale(1)' : 'scale(0.5)', transition: `all 0.6s ease-out ${i * 0.15}s` }}>
      ...
      <section id="about" className="px-4 lg:px-8" style={{ maxWidth: 1400, margin: '0 auto', paddingTop: 100, paddingBottom: 80 }}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.2em' }}>WHO WE ARE</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-16">
          {values.map(v => (
            <div key={v} className="text-center p-6" style={{
              border: `1px solid ${BORDER}`, borderRadius: 6, transition: 'all 0.3s',
            }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = TEAL; e.currentTarget.style.boxShadow = WARM_GLOW }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = BORDER; e.currentTarget.style.boxShadow = 'none' }}>
              <span style={{ ...gradTextStyle, fontSize: 15, fontWeight: 400 }}>{t(v)}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="clients" style={{ background: '#0a0a0a', paddingTop: 80, paddingBottom: 80 }}>
        <div className="px-4 lg:px-8" style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div className="text-center mb-12">
            <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.2em' }}>TRUSTED BY</span>
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 200, marginTop: 12 }}>{t('clients_title')}</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {clients.map(c => (
              <div key={c} className="text-center p-5" style={{
                border: `1px solid ${BORDER}`, borderRadius: 6, transition: 'all 0.3s', cursor: 'default',
              }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = TEAL; e.currentTarget.style.boxShadow = WARM_GLOW }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = BORDER; e.currentTarget.style.boxShadow = 'none' }}>
      ...
      <section id="contact" className="px-4 lg:px-8" style={{ maxWidth: 800, margin: '0 auto', paddingTop: 100, paddingBottom: 80 }}>
        <div className="text-center mb-12">
          <span style={{ ...gradTextStyle, fontSize: 11, letterSpacing: '0.2em' }}>GET IN TOUCH</span>
        ...
        {formSent && <div className="text-center p-4 mb-6" style={{ background: `rgba(45,212,191,0.08)`, border: `1px solid ${TEAL}`, borderRadius: 6, color: TEAL }}>{t('form_success')}</div>}
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={f.key}>
              ...
              <input type={f.type} required={f.key === 'full_name' || f.key === 'phone'}
                ...
                style={{ width: '100%', padding: '10px 14px', background: '#0a0a0a', border: `1px solid ${BORDER}`, borderRadius: 4, color: '#fff', fontSize: 14, outline: 'none', transition: 'border-color 0.3s' }}
                onFocus={e => (e.currentTarget.style.borderColor = TEAL)} onBlur={e => (e.currentTarget.style.borderColor = BORDER)} />
            </div>
          ))}
          <div>
            ...
            <select value={formData.service_type} onChange={e => setFormData({ ...formData, service_type: e.target.value })}
              style={{ width: '100%', padding: '10px 14px', background: '#0a0a0a', border: `1px solid ${BORDER}`, borderRadius: 4, color: '#fff', fontSize: 14, outline: 'none' }}>
          ...
          <div className="md:col-span-2">
            ...
            <textarea rows={4} value={formData.details} onChange={e => setFormData({ ...formData, details: e.target.value })}
              style={{ width: '100%', padding: '10px 14px', background: '#0a0a0a', border: `1px solid ${BORDER}`, borderRadius: 4, color: '#fff', fontSize: 14, outline: 'none', resize: 'vertical', transition: 'border-color 0.3s' }}
              onFocus={e => (e.currentTarget.style.borderColor = TEAL)} onBlur={e => (e.currentTarget.style.borderColor = BORDER)} />
          </div>
          <div className="md:col-span-2">
            <button type="submit" style={{
              width: '100%', padding: '14px', background: GRAD, color: '#fff', borderRadius: 4, fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer', letterSpacing: '0.15em',
              boxShadow: WARM_GLOW, transition: 'all 0.3s',
            }}
            onMouseEnter={e => { e.currentTarget.style.boxShadow = WARM_GLOW_HOVER }}
            onMouseLeave={e => { e.currentTarget.style.boxShadow = WARM_GLOW }}>
      ...
      {/* FOOTER */}
      <footer style={{ background: '#000', borderTop: `1px solid ${BORDER}`, paddingTop: 40, paddingBottom: 30 }}>
        <div className="px-4 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6" style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div className="flex items-center gap-6">
            <a href={settings.instagram} target="_blank" rel="noopener" style={{ color: '#555', textDecoration: 'none', fontSize: 13, transition: 'color 0.3s' }}
              onMouseEnter={e => (e.currentTarget.style.color = TEAL)} onMouseLeave={e => (e.currentTarget.style.color = '#555')}>Instagram</a>
            <a href={settings.twitter} target="_blank" rel="noopener" style={{ color: '#555', textDecoration: 'none', fontSize: 13, transition: 'color 0.3s' }}
              onMouseEnter={e => (e.currentTarget.style.color = TEAL)} onMouseLeave={e => (e.currentTarget.style.color = '#555')}>Twitter</a>
            <span style={{ color: '#555', fontSize: 13 }}>{settings.email}</span>
          </div>
          <Link to="/contact" style={{ ...gradTextStyle, fontSize: 12, textDecoration: 'none', letterSpacing: '0.1em' }}>CONTACT →</Link>
        </div>
      ...
      <a href={`https://wa.me/${settings.whatsapp}`} target="_blank" rel="noopener"
        style={{
          ...
          background: `linear-gradient(160deg, ${CORAL} 0%, #e03d75 25%, ${TEAL} 60%, ${BLUE_DEEP} 100%)`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: `0 4px 20px rgba(248,112,96,0.35), 0 2px 12px rgba(45,212,191,0.2)`,
          transition: 'transform 0.3s, box-shadow 0.3s',
        }}
        onMouseEnter={e => { const el = e.currentTarget; el.style.transform = 'scale(1.1)'; el.style.boxShadow = `0 6px 28px rgba(248,112,96,0.5), 0 4px 16px rgba(45,212,191,0.3)` }}
        onMouseLeave={e => { const el = e.currentTarget; el.style.transform = 'scale(1)'; el.style.boxShadow = `0 4px 20px rgba(248,112,96,0.35), 0 2px 12px rgba(45,212,191,0.2)` }}>
