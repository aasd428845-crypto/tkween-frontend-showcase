...
function saveProjects(p: TkweenProject[]) {
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <button onClick={() => setModal({ ...emptyProject })} style={{
          ...
          padding: '8px 20px', background: '#FF3B30', color: '#fff',
          borderRadius: 4, fontSize: 13, fontWeight: 500, border: 'none', cursor: 'pointer',
        }}>
              ...
              {projects.sort((a, b) => a.display_order - b.display_order).map(p => (
                <tr key={p.id} style={{ borderBottom: '1px solid #1c1c1c' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ padding: '3px 8px', borderRadius: 3, fontSize: 11, background: 'rgba(255,59,48,0.1)', color: '#FF3B30' }}>{p.category}</span>
                  </td>
                  <td style={{ padding: '12px 16px', color: p.video_url ? '#FF3B30' : '#555', fontSize: 13 }}>{p.video_url ? '✓' : '—'}</td>
                  <td style={{ padding: '12px 16px' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <button onClick={() => toggleVisible(p.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: p.visible ? '#FF3B30' : '#555' }}>
                      {p.visible ? <Eye size={18}/> : <EyeOff size={18}/>}
                    </button>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => setModal({ ...p })} style={{ padding: 6, background: 'rgba(96,165,250,0.1)', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#60a5fa' }}>
                        <Pencil size={14}/>
                      </button>
                      <button onClick={() => setConfirmDelete(p.id)} style={{ padding: 6, background: 'rgba(239,68,68,0.1)', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#ef4444' }}>
                        <Trash2 size={14}/>
                      </button>
                    </div>
                  </td>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ width: '100%', maxWidth: 500, padding: 24, background: '#0a0a0a', border: '1px solid #1c1c1c', borderRadius: 8 }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <select value={modal.category} onChange={e => setModal({ ...modal, category: e.target.value })} style={inputStyle}>
                  {['CONFERENCES', 'CORPORATE', 'BRAND', 'EVENTS'].map(c => <option key={c} value={c} style={{ background: '#111' }}>{c}</option>)}
                </select>
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button onClick={handleSave} style={{ flex: 1, padding: 10, background: '#FF3B30', color: '#fff', borderRadius: 4, fontSize: 14, fontWeight: 500, border: 'none', cursor: 'pointer' }}>{t('admin_save')}</button>
              <button onClick={() => setModal(null)} style={{ flex: 1, padding: 10, background: 'transparent', border: '1px solid #1c1c1c', color: '#888', borderRadius: 4, fontSize: 14, cursor: 'pointer' }}>{t('admin_cancel')}</button>
            </div>
          </div>
        </div>
      )}
