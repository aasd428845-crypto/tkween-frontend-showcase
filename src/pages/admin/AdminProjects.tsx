...
const emptyProject: Omit<Project, 'id' | 'created_at'> = {
  title_en: '', title_ar: '', category: 'CONFERENCES',
...
export default function AdminProjects() {
  ...
  useEffect(() => {
    apiGetProjects().then(data => { setProjects(data); setLoading(false) }).catch(() => setLoading(false))
  }, [])
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.id) {
        const { id, created_at, ...data } = modal as Project
        const updated = await apiUpdateProject(id, data)
        setProjects(prev => prev.map(p => p.id === id ? updated : p))
      } else {
        const created = await apiCreateProject(modal as Omit<Project, 'id' | 'created_at'>)
  ...
  const toggleFeatured = async (id: string) => {
    ...
    if (!p) return
    const updated = await apiUpdateProject(id, { featured: !p.featured })
  ...
  const toggleVisible = async (id: string) => {
    const p = projects.find(p => p.id === id)
    if (!p) return
    const updated = await apiUpdateProject(id, { visible: !p.visible })
    setProjects(prev => prev.map(x => x.id === id ? updated : x))
  }
                ...
                {projects.sort((a, b) => a.display_order - b.display_order).map(p => (
                  <tr key={p.id} style={{ borderBottom: `1px solid ${BORDER}` }}>
                    <td style={{ padding: '8px 16px' }}>
                      {p.thumbnail ? <img src={p.thumbnail} alt="" style={{ width: 60, height: 40, objectFit: 'cover', borderRadius: 3 }}/> : <div style={{ width: 60, height: 40, background: BG, borderRadius: 3 }}/>}
                    </td>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(4,10,6,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ width: '100%', maxWidth: 500, padding: 24, background: BG_SOFT, border: `1px solid ${BORDER}`, borderRadius: 8 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                { key: 'title_en', label: t('admin_title_en') },
                { key: 'title_ar', label: t('admin_title_ar') },
                { key: 'thumbnail', label: `${t('admin_thumbnail')} (direct image URL)` },
                { key: 'video_url', label: `${t('admin_video')} (Vimeo or direct URL)` },
              ].map(f => (
                <div key={f.key}>
              ...
              <div>
                ...
                <select value={modal.category || 'CONFERENCES'} onChange={e => setModal({ ...modal, category: e.target.value })} style={inputStyle}>
              ...
              <div>
                ...
                <input type="number" value={modal.display_order || 0} onChange={e => setModal({ ...modal, display_order: +e.target.value })} style={inputStyle}/>
              </div>
              <div style={{ display: 'flex', gap: 20 }}>
                {[{ k: 'featured', l: t('admin_featured') }, { k: 'visible', l: t('admin_visible') }].map(f => (
                  <label key={f.k} style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#888', fontSize: 13, cursor: 'pointer' }}>
                    <input type="checkbox" checked={(modal as any)[f.k] ?? false} onChange={e => setModal({ ...modal, [f.k]: e.target.checked })}/> {f.l}
                  </label>
                ))}
              </div>
            </div>
