...
const AdminDashboard = () => {
  ...
  const cards = [
    { icon: Film, label: 'admin_total_projects', value: projects.length, color: '#FF4500' },
  ...
  return (
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {cards.map(c => (
          <div key={c.label} className="p-5" style={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: 8 }}>
                ...
                <tr key={r.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ padding: '4px 10px', borderRadius: 4, fontSize: 11, fontWeight: 500,
                      background: r.status === 'new' ? 'rgba(245,158,11,0.15)' : r.status === 'reviewed' ? 'rgba(96,165,250,0.15)' : r.status === 'contacted' ? 'rgba(255,69,0,0.15)' : 'rgba(148,163,184,0.15)',
                      color: r.status === 'new' ? '#f59e0b' : r.status === 'reviewed' ? '#60a5fa' : r.status === 'contacted' ? '#FF4500' : '#94a3b8'
