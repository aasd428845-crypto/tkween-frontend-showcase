...
import { apiGetRequests, apiUpdateRequest, apiDeleteRequest } from '@/lib/api'
import type { Request as TkweenRequest } from '@/lib/api'
...
export default function AdminRequests() {
  ...
  useEffect(() => {
    apiGetRequests().then(data => { setRequests(data); setLoading(false) }).catch(() => setLoading(false))
  }, [])
  ...
  const updateStatus = async (id: string, status: string) => {
    const updated = await apiUpdateRequest(id, { status })
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
        {['ALL', 'new', 'reviewed', 'contacted', 'closed'].map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{
            ...
            background: filter === s ? GRAD : 'transparent',
            color: filter === s ? '#fff' : '#888', fontSize: 12, cursor: 'pointer',
          }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24, background: BG_SOFT, border: `1px solid ${BORDER}`, borderRadius: 6, padding: '0 12px' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', gap: 20 }}>
          <div style={{ flex: 1, background: BG_SOFT, border: `1px solid ${BORDER}`, borderRadius: 6, overflow: 'hidden' }}>
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    <tr key={r.id} onClick={() => setSelected(r)} style={{
                      ...
                      background: selected?.id === r.id ? `${GRAD_START}08` : 'transparent',
                    }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ padding: '3px 8px', borderRadius: 3, fontSize: 11, background: `${statusColors[r.status] || '#666'}20`, color: statusColors[r.status] || '#666' }}>
                          {t(`status_${r.status}`)}
                        </span>
                      </td>
                    </tr>
          ...
          {selected && (
            <div className="hidden lg:block" style={{ width: 340, background: BG_SOFT, border: `1px solid ${BORDER}`, borderRadius: 6, padding: 24, flexShrink: 0 }}>
              ...
              <button onClick={() => setConfirmDelete(selected.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#ef4444', background: 'none', border: 'none', fontSize: 13, cursor: 'pointer' }}>
