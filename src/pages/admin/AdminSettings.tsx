...
import { Save, Trash2, Plus, Image } from 'lucide-react';
...
const AdminSettings = () => {
  ...
  return (
    <AdminLayout>
      ...
      {/* Contact & Social */}
      <div className="mb-8 p-6" style={{ background: '#0d2420', border: '1px solid #1a3530', borderRadius: 12 }}>
        <h2 style={{ fontSize: 16, fontWeight: 400, color: '#2dd4bf', marginBottom: 20 }}>{t('admin_contact_social')}</h2>
          ...
          {contactFields.map(f => (
            <div key={f.key} className="flex gap-2">
              ...
              <button onClick={() => saveField(f.key)} className="self-end"
                style={{ padding: '10px 14px', background: saved === f.key ? '#2dd4bf' : 'rgba(45,212,191,0.1)', border: 'none', borderRadius: 8, cursor: 'pointer', color: saved === f.key ? '#0a1e1a' : '#2dd4bf', transition: 'all 0.3s' }}>
                <Save size={16} />
              </button>
            </div>
          ))}
      ...
      {/* Password */}
      <div className="mb-8 p-6" style={{ background: '#0d2420', border: '1px solid #1a3530', borderRadius: 12 }}>
        <h2 style={{ fontSize: 16, fontWeight: 400, color: '#2dd4bf', marginBottom: 20 }}>{t('admin_change_password')}</h2>
        <div className="flex gap-2 max-w-md">
          <input type="password" value={settings.admin_password} onChange={e => updateSetting('admin_password', e.target.value)}
            style={{ flex: 1, padding: '10px 14px', background: '#0a1e1a', border: '1px solid #1a3530', borderRadius: 8, color: '#fff', fontSize: 14, outline: 'none' }} />
          <button onClick={() => saveField('admin_password')}
            style={{ padding: '10px 14px', background: saved === 'admin_password' ? '#2dd4bf' : 'rgba(45,212,191,0.1)', border: 'none', borderRadius: 8, cursor: 'pointer', color: saved === 'admin_password' ? '#0a1e1a' : '#2dd4bf' }}>
            <Save size={16} />
          </button>
        </div>
      </div>
      ...
      {/* Visit Counter */}
      <div className="mb-8 p-6" style={{ background: '#0d2420', border: '1px solid #1a3530', borderRadius: 12 }}>
        <h2 style={{ fontSize: 16, fontWeight: 400, color: '#2dd4bf', marginBottom: 20 }}>{t('admin_visit_counter')}</h2>
        <div className="flex gap-2 max-w-md items-center">
          ...
          <button onClick={() => saveField('visit_count')}
            style={{ padding: '10px 14px', background: saved === 'visit_count' ? '#2dd4bf' : 'rgba(45,212,191,0.1)', border: 'none', borderRadius: 8, cursor: 'pointer', color: saved === 'visit_count' ? '#0a1e1a' : '#2dd4bf' }}>
            <Save size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="p-6" style={{ background: '#0d2420', border: '1px solid #1a3530', borderRadius: 12 }}>
        <h2 style={{ fontSize: 16, fontWeight: 400, color: '#2dd4bf', marginBottom: 20 }}>{t('admin_hero_images')}</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
          {heroImages.map((img, i) => (
            <div key={i} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', border: '1px solid #1a3530' }}>
        ...
        <div className="flex gap-2 max-w-lg">
          ...
          <button onClick={addHeroImage} className="flex items-center gap-2"
            style={{ padding: '10px 16px', background: '#2dd4bf', color: '#0a1e1a', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 500 }}>
            <Plus size={16} /> Add
          </button>
        </div>
      </div>
